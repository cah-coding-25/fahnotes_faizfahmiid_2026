...
export const GoogleSheetsModal: React.FC<GoogleSheetsModalProps> = ({
  ...
  const [copiedAppConfig, setCopiedAppConfig] = useState(false);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/60 backdrop-blur-xs">
      <div className="nb-box w-full max-w-2xl max-h-[92vh] flex flex-col overflow-hidden bg-white">
        ...
        <div className="flex border-b-2 border-black bg-[#FAF5EE] p-1.5 gap-1.5 overflow-x-auto">
          ...
          <button
            onClick={() => setActiveTab('github_vercel')}
            className={`nb-btn px-3.5 py-1.5 text-xs font-black gap-1.5 transition-all shrink-0 ${
              activeTab === 'github_vercel'
                ? 'bg-[#FF6584] text-white shadow-[2px_2px_0px_#000] border-2 border-black'
                : 'bg-white text-black/70 hover:text-black border-2 border-transparent'
            }`}
          >
            <Shield className="w-3.5 h-3.5 stroke-[2.5]" />
            <span>🚀 Simpan Permanen (GitHub &amp; Vercel)</span>
          </button>
        </div>
        ...
        <div className="p-4 sm:p-6 overflow-y-auto space-y-4 text-black text-xs font-semibold flex-1">
              ...
              <div className="p-4 rounded-xl border-2 border-black bg-white space-y-3 shadow-[3px_3px_0px_#000]">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="p-2 bg-[#F0FDF4] border border-emerald-300 rounded-lg text-[10px] text-emerald-950 font-bold leading-relaxed flex items-start gap-2">
                    ...
                    <span>
                      <strong>⚡ Sinkronisasi Global Aktif:</strong> Cukup input link di website ini dan klik "Simpan URL", sistem akan otomatis mengamankan dan menyinkronkannya ke cloud &amp; API server.
                    </span>
                  </div>

                  {webAppUrl && (
                    <div className="p-3 bg-[#EEF2FF] border-2 border-black rounded-xl space-y-2 shadow-[2px_2px_0px_#000]">
                      <div className="flex items-center justify-between">
                        <div className="text-[11px] font-black uppercase text-indigo-950 flex items-center gap-1.5">
                          <Sparkles className="w-3.5 h-3.5 text-indigo-600" />
                          <span>Link Auto-Connect (1-Klik untuk HP / Perangkat Lain):</span>
                        </div>
                        <button
                          type="button"
                          onClick={() => {
                            const link = generateAutoConnectLink(webAppUrl);
                            navigator.clipboard.writeText(link);
                            setCopiedMagicLink(true);
                            setTimeout(() => setCopiedMagicLink(false), 2500);
                          }}
                          className="nb-btn bg-[#818CF8] hover:bg-[#6366F1] text-white px-2.5 py-1 text-[11px] font-black gap-1 shadow-[2px_2px_0px_#000] border border-black"
                        >
                          {copiedMagicLink ? (
                            <>
                            ...
                            <>
                              ...
                              <span>Salin Link Auto-Connect</span>
                            </>
                          )}
                        </button>
                      </div>
                      <p className="text-[10px] font-bold text-indigo-900/80 leading-relaxed">
                        Kirim atau buka link ini di browser/HP lain: website akan langsung <strong>otomatis terhubung permanen</strong> ke database Spreadsheet tanpa perlu ketik ulang!
                      </p>
                    </div>
                  )}
                </div>
          ...
          {/* ========================================================================= */}
          {/* TAB 4: SIMPAN PERMANEN DI GITHUB & VERCEL (SOLUSI VERCEL HOSTING) */}
          {/* ========================================================================= */}
          {activeTab === 'github_vercel' && (
            <div className="space-y-4">
              
              {/* Alert Box Explaining Vercel / GitHub Behavior */}
              <div className="p-4 rounded-xl border-2 border-black bg-[#FFF7ED] space-y-2 shadow-[3px_3px_0px_#000]">
                <div className="flex items-center gap-2 font-black text-xs text-amber-950 uppercase tracking-wider">
                  ...
                  <span>Mengapa URL Terkadang Harus Diinput Ulang Saat Deploy di Vercel?</span>
                </div>
                <p className="text-[11px] font-bold text-amber-950/85 leading-relaxed">
                  Platform seperti <strong>Vercel</strong>, <strong>Netlify</strong>, dan <strong>GitHub Pages</strong> menyajikan website sebagai file statis (*static build*). Jika URL hanya disimpan di browser (*localStorage*), maka saat pengunjung lain membuka website Anda di perangkat berbeda, browser mereka masih kosong.
                </p>
                <div className="p-2.5 bg-white rounded-lg border border-amber-400 text-[10px] text-black font-bold">
                  💡 <strong>Solusi Permanen 100%:</strong> Simpan URL Anda langsung di dalam file <code>src/config/appConfig.ts</code> di GitHub repository pribadi Anda, ATAU setel di <strong>Vercel Environment Variables</strong>!
                </div>
              </div>

              {/* Status URL Saat Ini */}
              <div className="p-3.5 rounded-xl border-2 border-black bg-white space-y-2 shadow-[2px_2px_0px_#000]">
                <div className="text-[11px] font-black uppercase text-black flex items-center justify-between">
                  <span>URL Web App yang Sedang Aktif:</span>
                  <span className={`nb-badge text-[9px] ${webAppUrl ? 'bg-[#2DD4BF] text-black' : 'bg-[#FFE4E6] text-black'}`}>
                    {webAppUrl ? '🟢 Terdeteksi' : '🔴 Belum Diisi'}
                  </span>
                </div>
                <div className="p-2.5 bg-[#FAF5EE] rounded-lg border border-black font-mono text-[11px] break-all text-black">
                  {webAppUrl || '(Belum ada URL. Masukkan URL Google Apps Script Anda di tab Database terlebih dahulu)'}
                </div>
              </div>
...
export const APP_CONFIG = {
  ...
  SITE_NAME: 'fahnotes',
  AUTHOR_NAME: 'Faiz_Fahmi_ID',
  DEFAULT_CATEGORIES: ${JSON.stringify(categories, null, 4)}
};`}
                </pre>
              </div>

              {/* METODE 2: VERCEL ENVIRONMENT VARIABLES */}
              <div className="p-4 rounded-xl border-2 border-black bg-[#EFF6FF] space-y-3 shadow-[3px_3px_0px_#000]">
                <div className="flex items-center justify-between">
                  <div className="font-black text-xs uppercase tracking-wider text-blue-950 flex items-center gap-1.5">
                    <Zap className="w-4 h-4 text-blue-600 stroke-[2.5]" />
                    <span>Metode 2: Setel di Vercel Dashboard (Environment Variable)</span>
                  </div>
                  <button
                    onClick={() => {
                    ...
                    className="nb-btn bg-[#818CF8] hover:bg-[#9DA6FB] text-black px-3 py-1 text-xs font-black gap-1.5 shadow-[2px_2px_0px_#000]"
                  >
                    {copiedEnvVar ? (
                      <>
                      ...
                      <>
                        ...
                        <span>Salin Value URL</span>
                      </>
                    )}
                  </button>
                </div>

                <ol className="list-decimal list-inside text-[11px] font-bold text-blue-950/85 space-y-1">
                  <li>Buka Dashboard <strong>Vercel</strong> ➡️ Pilih Project Anda.</li>
                  <li>Klik <strong>Settings</strong> ➡️ <strong>Environment Variables</strong>.</li>
                  <li>Masukkan Key: <code>VITE_GOOGLE_SHEETS_URL</code></li>
                  <li>Masukkan Value: <code>{webAppUrl || 'https://script.google.com/macros/s/.../exec'}</code></li>
                  <li>Klik <strong>Save</strong> lalu lakukan <strong>Redeploy</strong> project Anda di Vercel.</li>
                </ol>
              </div>
